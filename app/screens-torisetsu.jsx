      ...
      <ToriSection title="強み">
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 10 }}>
          {TORI.strengths.map(s => (
            <span key={s} style={{ background: 'linear-gradient(135deg,#ff9d3f,var(--orange))', color: '#fff', fontFamily: 'var(--font-round)', fontWeight: 700, fontSize: 12, padding: '6px 13px', borderRadius: 999 }}>{s}</span>
          ))}
        </div>
...
function ToriHeroBooklet() {
  return (
    <div style={{ background: 'linear-gradient(150deg,#1aa6ff,#0069b5)', borderRadius: 'var(--r-lg)', padding: '22px 20px', color: '#fff', textAlign: 'center', boxShadow: 'var(--shadow-blue)', position: 'relative', overflow: 'hidden' }}>
...
function ToriHeroRadar() {
  return (
    <div className="card" style={{ padding: '18px 12px 10px', textAlign: 'center', background: 'linear-gradient(160deg,#fff,#eef7ff)' }}>
