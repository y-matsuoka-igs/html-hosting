...
function NextStepScreen() {
  const nav = useNav();
  const active = nav.state.tasks && nav.state.tasks.length ? nav.state.tasks : [
    { text: '得意なことを友だちにシェアしてみる', date: '2025.04.10', done: false },
    { text: '気になることにチャレンジしてみる', date: '2025.04.15', done: false },
  ];
  const [selected, setSelected] = useSf(null);
  const [custom, setCustom] = useSf('');
  const pending = active.filter(t => !t.done);

  const addTask = (text) => {
    const t = { text, date: todayStr(), done: false };
    nav.update(s => ({ tasks: [...(s.tasks && s.tasks.length ? s.tasks : active), t] }));
    setSelected(null); setCustom('');
  };
  const completeTask = (idx) => {
    const list = (nav.state.tasks && nav.state.tasks.length ? nav.state.tasks : active).map((t, i) => i === idx ? { ...t, done: true, completedDate: todayStr() } : t);
  ...
  return (
    <div className="screen">
      ...
      <div className="scroll pad stack">
        <div>
          <h2 style={{ fontSize: 17, fontWeight: 900 }}>チャレンジを決めよう</h2>
          <p style={{ fontSize: 12, color: 'var(--text-sub)', marginTop: 4, lineHeight: 1.7, fontWeight: 500 }}>小さな一歩でも大丈夫。選んでみよう！🌱</p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {NS_CHOICES.map((c, i) => {
            const on = selected === c;
            return (
              <button key={i} onClick={() => setSelected(on ? null : c)}
                style={{ textAlign: 'left', cursor: 'pointer', borderRadius: 'var(--r-md)', padding: '13px 15px', fontSize: 13, fontWeight: 600, color: on ? 'var(--blue-dark)' : 'var(--text)',
                  border: `2px solid ${on ? 'var(--blue)' : 'var(--border-soft)'}`, background: on ? 'var(--blue-soft)' : '#fff', transition: 'all .15s', WebkitTapHighlightColor: 'transparent' }}>
                {c}
              </button>
            );
          })}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn--primary" style={{ flex: 2 }} disabled={!selected} onClick={() => selected && addTask(selected.replace(/^\S+\s/, ''))}>この一歩を登録する</button>
          <button className="btn btn--outline" style={{ flex: 1 }} onClick={() => nav.go('ai-chat')}>AIに相談</button>
        </div>

        {/* custom input */}
        <div className="card card--flat">
          <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-sub)', marginBottom: 8 }}>✏️ 自分で入力する</div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
            ...
            <button onClick={() => custom.trim() && addTask(custom.trim())} style={{ flexShrink: 0, width: 44, height: 44, borderRadius: '50%', background: 'var(--blue)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
        ...
        {/* active challenges */}
        <h3 style={{ fontSize: 14, fontWeight: 800, marginTop: 2 }}>チャレンジ中</h3>
        {pending.length ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {(nav.state.tasks && nav.state.tasks.length ? nav.state.tasks : active).map((t, i) => t.done ? null : (
              <div key={i} className="card card--flat" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700 }}>{t.text}</div>
                  <div style={{ fontSize: 10, color: 'var(--text-sub)', marginTop: 2 }}>登録日: {t.date}</div>
                </div>
                <button onClick={() => completeTask(i)} className="btn btn--green btn--sm" style={{ width: 'auto', padding: '8px 14px' }}>できた！</button>
              </div>
            ))}
          </div>
        ) : (
          <p style={{ fontSize: 12, color: 'var(--text-sub)', textAlign: 'center', padding: '12px 0' }}>チャレンジ中の一歩がなくなりました！🎉<br/>新しい一歩を登録しよう</p>
        )}

        <button className="btn btn--ghost" onClick={() => nav.go('tree')} style={{ gap: 8 }}>
          <Icon name="tree" size={16} /> これまで完了したチャレンジを見る
        </button>
      </div>
    </div>
  );
...
/* ════════ 完了の木（EXP） ════════ */
const EVO_STAGES = [
  { emoji: '🌱', name: 'めばえの冒険者', lv: 'Lv.1-2', note: '最初の一歩を踏み出した！' },
  { emoji: '🌿', name: '成長の探求者',   lv: 'Lv.3-5', note: 'あと少しでアンロック！' },
  { emoji: '🌳', name: '挑戦のパイオニア', lv: 'Lv.6-9', note: '' },
  { emoji: '⭐', name: '伝説の可能性開拓者', lv: 'Lv.10+', note: '' },
];
    ...
    <div className="screen">
      ...
      <div className="scroll pad stack">
        ...
        <div style={{ background: 'linear-gradient(135deg,#1aa6ff,#0069b5)', borderRadius: 'var(--r-lg)', padding: 16, color: '#fff', display: 'flex', alignItems: 'center', gap: 14, boxShadow: 'var(--shadow-blue)' }}>
          <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'rgba(255,255,255,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 30 }}>{stage.emoji}</div>
          <div style={{ flex: 1 }}>
        ...
        <div className="card">
          ...
          <div style={{ fontSize: 10, color: 'var(--text-sub)', marginTop: 6 }}>一歩を完了するたびにEXPが増えます</div>
        </div>
        ...
        <div className="card">
          ...
          {EVO_STAGES.map((s, i) => {
            const isCur = i === stageIdx, passed = i < stageIdx, last = i === EVO_STAGES.length - 1;
            const op = passed || isCur ? 1 : 0.4;
            return (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, opacity: op,
                    ...
                    border: `2px solid ${isCur ? 'var(--blue)' : passed ? 'var(--green)' : 'var(--border)'}` }}>{s.emoji}</div>
        ...
        {/* completed list */}
        <h3 style={{ fontSize: 14, fontWeight: 800 }}>完了したチャレンジ</h3>
        ...
        <button className="btn btn--ghost" onClick={() => nav.go('next-step')}>← チャレンジに戻る</button>
      </div>
    </div>
  );
}

Object.assign(window, { AiChatScreen, NextStepScreen, TreeScreen });
